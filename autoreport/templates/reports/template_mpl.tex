...
% 图片统一从 Plotting 的真实输出目录解析，正文里直接用文件名即可，无需复制或建软链接。
\graphicspath{{../Plots/Fig/}}
% 允许 \begin{figure}[H] / \begin{table}[H] 把图表强制固定在源码位置（不浮动）。
\usepackage{float}
...
\begin{figure}
  \centering
  \includegraphics[width=0.85\linewidth]{instruments.png}
...
\begin{figure}
  \centering
  \includegraphics[width=0.8\textwidth]{figsample.pdf}
  \caption{这是数据图的例子.
