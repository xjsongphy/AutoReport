...
async def test_reply_completes_task_and_publishes_report(board, bus):
    ...
    result = await tool(task_id="tk1", type="reply", content="done, see Plots/Fig/a.png")
...
async def test_reply_is_idempotent_for_already_completed_task(board, bus):
    ...
    result = await tool(task_id="tk1", type="reply", content="done again")

    assert result["status"] == "ok"
    published = [await bus._queue.get()]
    assert any(isinstance(m, ReportMessage) and m.task_id == "tk1" for m in published)
